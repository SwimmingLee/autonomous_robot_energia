...
cIMU::cIMU()
{
  ...
  for( i=0; i<3; i++ )
  {
    rpy[i] = 0.; //rpy가 무엇이길래 처어므부터 0으로 초기화를 해주느가>?
...
/*---------------------------------------------------------------------------
     TITLE   : begin
     WORK    : Resolution 초기화, 연결하기
     ARG     : void
     RET     : void
---------------------------------------------------------------------------*/
uint8_t cIMU::begin( uint32_t hz )
{
	uint8_t err_code = IMU_OK; //0x00
  uint32_t i;
  uint32_t pre_time;
  ...
  bConnected = SEN.begin(); //mpu9250과 연결하기 

  if( bConnected == true )
  {
    filter.begin(update_hz);

    for (i=0; i<32; i++) //왜 업데이트를 32번이나 해주는건가 
    {
      update();
    }
...
//일종의 low-pass 필터라고 이렇게 코딩한 것 같다 .
  ...
  if (SEN.calibratingG == 0 && SEN.calibratingA == 0)
  {
    ...
    //어째서 지자기 센서는 사용하지 않은것인가?
  }

  //아마도 퀀터니언을 오일러 앵글로 바꾸는 과정일 것이다.
